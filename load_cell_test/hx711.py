...
class HX711:
    ...
    def __init__(self, dout=5, pd_sck=6, gain=128, dout_pin=None, pd_sck_pin=None):
        self.PD_SCK = pd_sck_pin if pd_sck_pin is not None else pd_sck
        self.DOUT = dout_pin if dout_pin is not None else dout
    ...
    def read_raw(self, times=5):
        """Read median raw value over multiple samples."""
        return int(self.read_median(times))

    def read_weight(self, times=5):
        """Read weight in user units (grams) over multiple samples."""
        return round(float(self.get_weight(times)), 1)

    def set_scale(self, scale):
        """Set calibration scale factor (alias for set_reference_unit)."""
